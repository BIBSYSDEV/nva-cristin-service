...
class CreateKeywordHandlerTest {
  ...
  void shouldThrowExceptionWhenInputNotValid(Keyword input)
    ...
    assertThat(
        gatewayResponse.getBody(), containsString(CristinCreateKeywordValidator.ERROR_MESSAGE));
  }
  ...
  private static Stream<Arguments> badRequestProvider() {
    return Stream.of(
        Arguments.of(new Keyword.Builder().build()),
  ...
  private GatewayResponse<Keyword> sendQueryWithoutAccessRights(Keyword body) throws IOException {
    var input = new HandlerRequestBuilder<>(OBJECT_MAPPER).withBody(body).build();
