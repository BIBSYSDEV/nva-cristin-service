...
class UpdateCristinProjectHandlerTest {
  ...
  private static final Map<String, String> validPath =
      Map.of(PROJECT_IDENTIFIER, randomIntegerAsString());
  ...
  void shouldReturnBadRequestOnInvalidJson(String field, JsonNode input, String exceptionMessage)
      throws Exception {
    handler = spy(handler);
    doReturn(true).when(handler).hasResourceAccess(any());
  ...
  void shouldProduceCorrectPayloadToSendToCristin() throws Exception {
    ...
    assertThat(
        OBJECT_MAPPER.readTree(actualPayload), equalTo(OBJECT_MAPPER.readTree(expectedPayload)));
  }

  @ParameterizedTest(name = "Allowing null value for field {0}")
  @ValueSource(
      strings = {"funding", "relatedProjects", "institutionsResponsibleForResearch", "webPage"})
  ...
  @Test
  void shouldDefaultToEmptyStringAndNotCrashWhenSendingNonTextualValuesForContactInfoFields()
      throws Exception {
    handler = spy(handler);
    doReturn(true).when(handler).hasResourceAccess(any());
  ...
  void shouldNotAllowPersonWithoutAccessRightOrRoleInProjectToEditRequestedProject()
    ...
    var fetchedProjectJson =
        cristinProjectWithParticipantRoles(ANOTHER_IDENTIFIER_THAN_USER, CRISTIN_PRO_MANAGER_CODE)
            .toString();
    mockFetchResponse(fetchedProjectJson);
  ...
  @Test
  void shouldNotAllowPersonWhichIsCreatorAndManagerButWithoutRoleToEditTheirOwnProject()
      throws Exception {
  ...
  private void mockFetchResponse(String fetchedProjectJson)
      throws IOException, InterruptedException {
    doReturn(new HttpResponseFaker(fetchedProjectJson, HTTP_OK))
        .when(httpClientMockFetch)
        .<String>send(any(), any());
  }
  ...
  private InputStream createRequest(String body, AccessRight accessRight)
      throws JsonProcessingException {
    var customerId = randomUri();
    return new HandlerRequestBuilder<String>(OBJECT_MAPPER)
        .withBody(body)
  ...
  private static Stream<Arguments> badRequestProvider() {
    return Stream.of(
        ...
        Arguments.of(
            FUNDING, fundingWithoutRequiredFields(), FUNDING_MISSING_REQUIRED_FIELDS_OR_NOT_VALID),
        Arguments.of(
            LANGUAGE, languageNotSupported(), format(ILLEGAL_VALUE_FOR_PROPERTY, LANGUAGE)),
        ...
        Arguments.of(
            KEYWORDS, missingRequiredFieldType(KEYWORDS), KEYWORDS_MISSING_REQUIRED_FIELD_TYPE),
        Arguments.of(
            PROJECT_CATEGORIES,
            missingRequiredFieldType(PROJECT_CATEGORIES),
            PROJECT_CATEGORIES_MISSING_REQUIRED_FIELD_TYPE),
        Arguments.of(
            RELATED_PROJECTS,
            notAnArray(RELATED_PROJECTS),
            format(MUST_BE_A_LIST, RELATED_PROJECTS)),
        Arguments.of(RELATED_PROJECTS, notListOfStrings(), MUST_BE_A_LIST_OF_IDENTIFIERS),
        Arguments.of(
            ACADEMIC_SUMMARY,
            notADescription(ACADEMIC_SUMMARY),
            format(NOT_A_VALID_KEY_VALUE_FIELD, ACADEMIC_SUMMARY)),
        Arguments.of(
            POPULAR_SCIENTIFIC_SUMMARY,
            notADescription(POPULAR_SCIENTIFIC_SUMMARY),
            format(NOT_A_VALID_KEY_VALUE_FIELD, POPULAR_SCIENTIFIC_SUMMARY)),
        Arguments.of(METHOD, notADescription(METHOD), format(NOT_A_VALID_KEY_VALUE_FIELD, METHOD)),
        Arguments.of(
            EQUIPMENT, notADescription(EQUIPMENT), format(NOT_A_VALID_KEY_VALUE_FIELD, EQUIPMENT)),
        Arguments.of(
            NVA_INSTITUTIONS_RESPONSIBLE_FOR_RESEARCH,
            notAnOrganization(),
        ...
        Arguments.of(
            END_DATE, endDateInstant(EMPTY_STRING), format(ILLEGAL_VALUE_FOR_PROPERTY, END_DATE)),
        Arguments.of(END_DATE, endDateInstant(null), format(ILLEGAL_VALUE_FOR_PROPERTY, END_DATE)),
        Arguments.of(
            END_DATE,
            endDateInstant(randomString()),
            format(ILLEGAL_VALUE_FOR_PROPERTY, END_DATE)));
  }
  ...
  private static JsonNode languageNotAValidValue() {
    return OBJECT_MAPPER
        .createObjectNode()
        .putPOJO(LANGUAGE, List.of(randomInteger(), randomInteger()));
  }
  ...
  private CristinProject cristinProjectWithParticipantRoles(
      String userIdentifier, String roleCode) {
    var cristinProject = basicCristinProject();
  ...
  private CristinProject cristinProjectWithManagerAndCreatorAsTheSame() {
    ...
    cristinProject.setParticipants(
        List.of(getParticipant(USER_IDENTIFIER, CRISTIN_PRO_MANAGER_CODE)));
    return cristinProject;
  }
}
