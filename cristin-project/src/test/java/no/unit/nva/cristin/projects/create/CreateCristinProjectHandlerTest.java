...
class CreateCristinProjectHandlerTest {
  ...
  public static final String SUPPLIED_CLINICAL_TRIAL_PHASE_IS_NOT_VALID =
      "Supplied ClinicalTrialPhase is not valid";
  public static final String HEALTH_PROJECT_TYPE_JSON_FIELD = "\"type\": \"%s\"";
  public static final String SUPPLIED_HEALTH_PROJECT_TYPE_IS_NOT_VALID =
      "Supplied HealthProjectType is not valid";
  public static final boolean IS_EXEMPT_FROM_PUBLIC_DISCLOSURE = true;
  public static final URI CRISTIN_PERSON_ID =
      URI.create("https://api.dev.nva.aws.unit.no/cristin/person/12345");
  ...
  void shouldReturn403ForbiddenWhenRequestIsMissingRole() throws Exception {
    ...
    var input =
        new HandlerRequestBuilder<NvaProject>(OBJECT_MAPPER)
  ...
  void shouldReturn400BadRequestWhenMissingFields() throws Exception {
    ...
    assertThat(
        response.getBody(),
        containsString(ValidatedResult.HasNoCoordinatingOrganization.getLabel()));
    assertThat(response.getBody(), containsString(ValidatedResult.HasNoContributors.getLabel()));
    assertThat(response.getBody(), containsString(ValidatedResult.InvalidStartDate.getLabel()));
  ...
  @Test
  void shouldReturnProjectDataWhenCreatingWithUnitIdentifier()
  ...
  void shouldReturnBadRequestWhenHealthProjectDataTypePresentButInvalid() throws Exception {
    ...
    var expected =
        json.replace(
            String.format(jsonToReplace, DRUGSTUDY.getType()),
  ...
  @Test
  void shouldReturnBadRequestWhenHealthProjectDataClinicalPhasePresentButInvalid()
      throws Exception {
    var json = getProjectPostRequestJsonSample();
    ...
    var expected =
        json.replace(
            String.format(jsonToReplace, PHASE_THREE.getPhase()),
            String.format(jsonToReplace, INVALIDVALUE));
  ...
  void shouldHaveProjectWithHealthDataAddedAndSentToUpstream() throws Exception {
    ...
    assertThat(
        cristinHealthType, equalTo(CristinHealthProjectTypeBuilder.reverseLookup(nvaHealthType)));
  ...
  private Approval approvalWithOnlySomeFieldsPopulated() {
    return new Approval(
        randomInstant(),
        ApprovalAuthority.NORWEGIAN_DIRECTORATE_OF_HEALTH,
        null,
        null,
        randomString(),
        null);
  ...
  void shouldHaveProjectWithExemptFromPublicDisclosureAddedAndSentToUpstream() throws Exception {
    ...
    assertThat(
        capturedCristinProject.getExemptFromPublicDisclosure(),
        equalTo(IS_EXEMPT_FROM_PUBLIC_DISCLOSURE));
  }
  ...
  private boolean hasRole(CristinProject capturedCristinProject, String roleCode) {
    return capturedCristinProject.getParticipants().stream()
        .anyMatch(person -> person.roles().get(0).getRoleCode().equals(roleCode));
  }

  private InputStream inputWithClientIdentifiers(NvaProject nvaProject)
  ...
  private CristinProject captureCristinProjectFromApiClient(
      CreateCristinProjectApiClient apiClient, NvaProject nvaProject) throws Exception {
    mockUpstreamUsingRequest(nvaProject);
    executeRequest(nvaProject);
  ...
  private void mockUpstreamUsingRequest(NvaProject request)
      throws IOException, InterruptedException {
  ...
  private void addMockedResponseFieldsToCristinProject(
      CristinProject cristinProject, NvaProject request) {
  ...
  private static InputStream getInputStreamFromString(String expected)
      throws JsonProcessingException {
    var customerId = randomUri();
