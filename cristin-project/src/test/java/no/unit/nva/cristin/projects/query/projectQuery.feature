Feature: API tests for Cristin projects query
  ...
  Scenario: Query with bad parameter returns Bad Request
    ...
    * def contentType = responseHeaders['Content-Type'][0]
    ...
    And match response.detail == 'Invalid query parameter supplied. Valid parameters: [\'approval_reference_id\', \'approved_by\', \'biobank\', \'depth\', \'funding\', \'funding_source\', \'institution\', \'keyword\', \'modified_since\', \'name\', \'organization\', \'page\', \'participant\', \'project_code\', \'project_manager\', \'query\', \'results\', \'sort\', \'status\', \'title\', \'unit\', \'user\']'
    And match response.requestId == '#notnull'
