...
public class QueryProjectTest {
  ...
  public static final ParameterKeyProject[] QUERY_PARAMETER_KEYS = {
    PAGE_CURRENT, PAGE_ITEMS_PER_PAGE
  };
  ...
  void buildReturnsUriWithIdAndLanguageWhenIdAndLanguageParametersAreSupplied()
    ...
    var uri2 = QueryProject.builder().withPathIdentity(ID).build().toURI();
  ...
  @Test
  void buildReturnsUriCorrectlyAndWithoutDoingDoubleEncodingWhenHavingMoreOfSameParam()
      throws BadRequestException {
    var uri =
        QueryProject.builder()
            .withKeyword(KEYWORD)
