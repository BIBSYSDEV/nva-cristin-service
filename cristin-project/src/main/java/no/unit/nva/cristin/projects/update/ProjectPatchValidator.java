...
public class ProjectPatchValidator extends PatchValidator implements Validator<ObjectNode> {
  ...
  public static final String FUNDING_MISSING_REQUIRED_FIELDS_OR_NOT_VALID =
      "Funding missing required fields or not" + " valid";
  public static final String MUST_BE_A_LIST = "Field %s must be a list";
  public static final String KEYWORDS_MISSING_REQUIRED_FIELD_TYPE =
      "Keywords missing required field 'type'";
  public static final String PROJECT_CATEGORIES_MISSING_REQUIRED_FIELD_TYPE =
      "ProjectCategories missing required " + "field 'type'";
  public static final String MUST_BE_A_LIST_OF_IDENTIFIERS =
      "RelatedProjects must be a list of identifiers, " + "numeric or URI";
  public static final String NOT_A_VALID_LIST_OF_KEY_VALUE_FIELDS =
      "%s not a valid list of key value fields";
      ...
      attempt(
              () ->
                  OBJECT_MAPPER.readValue(
                      input.get(COORDINATING_INSTITUTION).toString(), Organization.class))
          .orElseThrow(
              fail ->
                  new BadRequestException(
                      format(ILLEGAL_VALUE_FOR_PROPERTY, COORDINATING_INSTITUTION)));
    }
  }
  ...
  private static void validateJsonReadable(
      TypeReference<?> typeRef, String content, String propertyName) throws BadRequestException {
    attempt(() -> OBJECT_MAPPER.readValue(content, typeRef))
        .orElseThrow(
            fail -> new BadRequestException(format(ILLEGAL_VALUE_FOR_PROPERTY, propertyName)));
  }
