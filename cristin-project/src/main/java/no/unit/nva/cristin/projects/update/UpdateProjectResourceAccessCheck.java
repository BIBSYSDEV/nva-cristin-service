...
public class UpdateProjectResourceAccessCheck extends ProjectResourceAccessCheck
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(UpdateProjectResourceAccessCheck.class);
  ...
  public void verifyAccess(NvaProject resource, Map<String, String> params) {
    ...
    getResourceCreator(resource)
        .ifPresentOrElse(
            creator -> verified = hasMatch(creator, userIdentifier), () -> verified = false);
    if (verified) {
      return;
    }

    getResourceManager(resource).ifPresent(manager -> verified = hasMatch(manager, userIdentifier));
    if (verified) {
      return;
    }
  ...
  private Optional<String> getResourceManager(NvaProject resource) {
    return Optional.ofNullable(resource).map(NvaProject::getContributors).stream()
