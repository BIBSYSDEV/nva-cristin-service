...
public class QueryProjectWithFacetsClient extends CristinProjectApiClient
  ...
  @Override
  public SearchResponse<NvaProject> executeQuery(QueryProject queryProject)
    ...
    return new SearchResponse<NvaProject>(id)
        ...
        .usingHeadersAndQueryParams(
            response.headers(), queryProject.toNvaParametersWithAddedFacets())
  ...
  private CristinProjectSearchResponse deserializeResponse(HttpResponse<String> response)
  ...
  /**
   * Enrich list of CristinProject from query with more data by doing one extra lookup request per
   * project.
   */
