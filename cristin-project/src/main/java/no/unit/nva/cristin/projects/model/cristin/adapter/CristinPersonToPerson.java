...
public class CristinPersonToPerson implements Function<CristinPerson, Person> {
  ...
  public Person apply(CristinPerson cristinPerson) {
    return Optional.ofNullable(cristinPerson).map(this::convert).orElse(null);
  }

  private Person convert(CristinPerson cristinPerson) {
    return new Person(
        extractIdIfIdentified(
            cristinPerson.identifiedCristinPerson(), cristinPerson.cristinPersonId()),
