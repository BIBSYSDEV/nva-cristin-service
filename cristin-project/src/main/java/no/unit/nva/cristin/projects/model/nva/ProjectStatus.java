...
public enum ProjectStatus {
  ...
  public static final String ERROR_MESSAGE_TEMPLATE =
      "%s not a valid ProjectStatus, expected one of: %s";
  ...
  public static ProjectStatus fromCristinStatus(String name) {
    return stream(values())
        .filter(
            nameType ->
                nameType.getCristinStatus().equalsIgnoreCase(name)
  ...
  private static RuntimeException returnException(String name) {
    return new IllegalArgumentException(
        format(
            ...
            name,
            stream(values()).map(ProjectStatus::toString).collect(joining(DELIMITER))));
  }
