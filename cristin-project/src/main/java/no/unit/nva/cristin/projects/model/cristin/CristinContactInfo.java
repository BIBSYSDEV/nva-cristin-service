...
public record CristinContactInfo(
    @JsonProperty(CRISTIN_CONTACT_PERSON) String contactPerson,
  ...
  public static CristinContactInfo fromContactInfo(ContactInfo contactInfo) {
    return Optional.ofNullable(contactInfo).map(CristinContactInfo::convert).orElse(null);
  }

  private static CristinContactInfo convert(ContactInfo contactInfo) {
    return new CristinContactInfo(
  ...
  public ContactInfo toContactInfo() {
    return new ContactInfo(contactPerson(), institution(), email(), phone());
  }
}
