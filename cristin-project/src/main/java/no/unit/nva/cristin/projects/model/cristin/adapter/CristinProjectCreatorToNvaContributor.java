...
public class CristinProjectCreatorToNvaContributor
  ...
  private NvaContributor extractCreator(CristinPerson creator) {
    return Optional.ofNullable(creator).stream()
  ...
  private CristinPerson addRoleDataIfMissing(CristinPerson cristinPerson) {
    var roles =
        Optional.ofNullable(cristinPerson.roles())
            .map(this::addProjectCreatorRoleCode)
            .orElse(Collections.emptyList());

    return cristinPerson.copy().withRoles(roles).build();
  }

  private List<CristinRole> addProjectCreatorRoleCode(List<CristinRole> cristinRoles) {
    return cristinRoles.stream().peek(role -> role.setRoleCode(PRO_CREATOR)).toList();
  }
