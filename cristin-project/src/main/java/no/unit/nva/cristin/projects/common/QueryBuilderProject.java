...
public class QueryBuilderProject extends QueryBuilder<ParameterKeyProject> {
  ...
  @Override
  protected void throwInvalidParamererValue(Entry<ParameterKeyProject, String> entry)
    ...
    if (invalidQueryParameter(key, entry.getValue())) {
      ...
      if (key == STATUS) {
        errorMessage =
            invalidQueryParametersMessageWithRange(
                key.getKey(), Arrays.toString(ProjectStatus.values()));
      } else {
  ...
  public QueryBuilderProject withBiobank(String biobank) {
    var biobanks = query.containsKey(BIOBANK) ? query.getValue(BIOBANK) + "," + biobank : biobank;
  ...
  public QueryBuilderProject withKeyword(String keyword) {
    var keywords = query.containsKey(KEYWORD) ? query.getValue(KEYWORD) + "," + keyword : keyword;
  ...
  public QueryBuilderProject withCategory(String category) {
    var categories =
        query.containsKey(CATEGORY) ? query.getValue(CATEGORY) + "," + category : category;
  ...
  public QueryBuilderProject withStatus(String status) {
    var statusKind =
        attempt(() -> ProjectStatus.valueOf(status.toUpperCase(Locale.getDefault())).name())
