...
public class CristinProjectPatchJsonCreator {
  ...
  /**
   * Class for creating json matching Cristin schema for Project update. Aktuelle kandidater: title
   * coordinating_institution institutions_responsible_for_research start_date end_date participants
   */
  ...
  private void addContactInfoIfPresent() {
    if (input.has(CONTACT_INFO) && !input.get(CONTACT_INFO).isNull()) {
      ...
      addNullableStringField(
          contactInfoInput, contactInfoOutput, CONTACT_PERSON, CRISTIN_CONTACT_PERSON);
  ...
  private void addNullableStringField(
      JsonNode input, ObjectNode newField, String fieldName, String cristinFieldName) {
  ...
  private void addResponsibleInstitutionsIfPresent() {
    if (input.has(NVA_INSTITUTIONS_RESPONSIBLE_FOR_RESEARCH)) {
      ...
      output.set(
          INSTITUTIONS_RESPONSIBLE_FOR_RESEARCH, OBJECT_MAPPER.valueToTree(cristinOrganizations));
    }
  }
