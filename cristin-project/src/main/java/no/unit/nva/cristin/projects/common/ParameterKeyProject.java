...
public enum ParameterKeyProject implements IParameterKey {
  ...
  QUERY(
      ...
      String.format(ERROR_MESSAGE_INVALID_VALUE, JsonPropertyNames.QUERY),
      KeyEncoding.NONE),
  ...
  TITLE(
      ...
      null,
      PATTERN_IS_NON_EMPTY,
      String.format(ERROR_MESSAGE_INVALID_VALUE, JsonPropertyNames.TITLE),
      KeyEncoding.NONE),
  USER(JsonPropertyNames.USER),
  PAGE_CURRENT(
      JsonPropertyNames.PAGE,
      null,
      PATTERN_IS_NUMBER,
      ERROR_MESSAGE_INVALID_NUMBER,
      KeyEncoding.NONE),
  PAGE_ITEMS_PER_PAGE(
      CRISTIN_PER_PAGE_PARAM,
  ...
  CRISTIN_ID(
      CRISTIN_ID_CRISTIN_PARAM,
      CRISTIN_ID_PARAM,
      PATTERN_IS_NUMBER,
      ERROR_MESSAGE_INVALID_NUMBER,
      KeyEncoding.NONE),
  MULTIPLE(MULTIPLE_PARAM),
  // Facets from here onward. For each new param above this line, increment
  // IGNORE_FACET_PARAMETER_INDEX by 1
  SECTOR_FACET(
      CristinFacetParamKey.SECTOR_PARAM.getKey(), CristinFacetParamKey.SECTOR_PARAM.getNvaKey()),
  COORDINATING_FACET(
      CristinFacetParamKey.COORDINATING_PARAM.getKey(),
      CristinFacetParamKey.COORDINATING_PARAM.getNvaKey()),
  RESPONSIBLE_FACET(
      CristinFacetParamKey.RESPONSIBLE_PARAM.getKey(),
      CristinFacetParamKey.RESPONSIBLE_PARAM.getNvaKey()),
  CATEGORY_FACET(
      CristinFacetParamKey.CATEGORY_PARAM.getKey(),
      CristinFacetParamKey.CATEGORY_PARAM.getNvaKey()),
  HEALTH_FACET(
      CristinFacetParamKey.HEALTH_PARAM.getKey(), CristinFacetParamKey.HEALTH_PARAM.getNvaKey()),
  PARTICIPANT_FACET(
      CristinFacetParamKey.PARTICIPANT_PARAM.getKey(),
      CristinFacetParamKey.PARTICIPANT_PARAM.getNvaKey()),
  PARTICIPATING_PERSON_ORG_FACET(
      CristinFacetParamKey.PARTICIPATING_PERSON_ORG_PARAM.getKey(),
      CristinFacetParamKey.PARTICIPATING_PERSON_ORG_PARAM.getNvaKey()),
  FUNDING_SOURCE_FACET(
      CristinFacetParamKey.FUNDING_SOURCE_PARAM.getKey(),
  ...
  ParameterKeyProject(
      String cristinKey, String nvaKey, String pattern, String errorMessage, KeyEncoding encode) {
  ...
  public String toString() {
    return new StringJoiner(":", "Key[", "]").add(String.valueOf(ordinal())).add(name()).toString();
  }
