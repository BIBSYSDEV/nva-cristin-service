...
public class CristinPersonsToNvaContributors
    implements Function<Collection<CristinPerson>, List<NvaContributor>> {

  @Override
  public List<NvaContributor> apply(Collection<CristinPerson> cristinPersons) {
    return Optional.ofNullable(cristinPersons).map(this::convert).orElse(null);
  }

  private List<NvaContributor> convert(Collection<CristinPerson> cristinPersons) {
    return cristinPersons.stream().map(new CristinPersonToNvaContributor()).toList();
  }
}
