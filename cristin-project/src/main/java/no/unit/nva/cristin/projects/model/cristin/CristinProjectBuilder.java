...
public class CristinProjectBuilder implements Function<NvaProject, CristinProject> {
  ...
  private CristinProject build() {
    ...
    cristinProject.setPopularScientificSummary(
        extractSummary(nvaProject.getPopularScientificSummary()));
    ...
    cristinProject.setCoordinatingInstitution(
        extractCristinOrganization(nvaProject.getCoordinatingInstitution()));
    ...
    cristinProject.setInstitutionsResponsibleForResearch(
        extractInstitutionsResponsibleForResearch(
            nvaProject.getInstitutionsResponsibleForResearch()));
    cristinProject.setHealthProjectType(
        extractHealthProjectType(nvaProject.getHealthProjectData()));
    cristinProject.setHealthProjectTypeName(
        extractHealthProjectTypeName(nvaProject.getHealthProjectData()));
    cristinProject.setClinicalTrialPhase(
        extractHealthProjectClinicalTrialPhase(nvaProject.getHealthProjectData()));
    ...
    cristinProject.setProjectCategories(
        extractCristinTypedLabels(nvaProject.getProjectCategories()));
  ...
  private CristinPerson extractCreator(NvaContributor creator) {
    return Optional.ofNullable(creator).map(this::getCreator).orElse(null);
  }

  private CristinPerson getCreator(NvaContributor creator) {
    return Optional.of(creator.roles()).stream()
  ...
  private List<CristinApproval> extractApprovals(List<Approval> approvals) {
    return approvals.stream().map(new ApprovalToCristinApproval()).collect(Collectors.toList());
  }
  ...
  private List<CristinTypedLabel> extractCristinTypedLabels(List<TypedLabel> typedLabels) {
    return typedLabels.stream().map(new TypedLabelToCristinFormat()).collect(Collectors.toList());
  }
  ...
  private List<CristinFundingSource> extractFundings(List<Funding> fundings) {
    return fundings.stream().map(CristinFundingSource::fromFunding).collect(Collectors.toList());
  }
