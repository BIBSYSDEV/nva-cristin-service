...
public class NvaProjectBuilder implements Function<CristinProject, NvaProject> {
  ...
  private DateInfo extractDateInfo(CristinDateInfo cristinDateInfo) {
    return Optional.ofNullable(cristinDateInfo).map(CristinDateInfo::toDateInfo).orElse(null);
  }
  ...
  private List<TypedLabel> convertCristinTypedLabels(
      Collection<CristinTypedLabel> cristinTypedLabels) {
    return cristinTypedLabels.stream().map(new CristinTypedLabelToNvaFormat()).toList();
  }
  ...
  private List<Approval> extractApprovals(Collection<CristinApproval> cristinApprovals) {
    return cristinApprovals.stream().map(new CristinApprovalToApproval()).toList();
  }

  private List<Organization> extractInstitutionsResponsibleForResearch(
      Collection<CristinOrganization> institutionsResponsibleForResearch) {
