...
public class ListFundingSourcesHandler extends CristinQueryHandler<Void, FundingSources> {
  ...
  private static final URI CONTEXT_URI =
      UriWrapper.fromUri("https://bibsysdev.github.io/src/funding-context.json").getUri();
  ...
  public ListFundingSourcesHandler(
      CristinFundingSourcesApiClient cristinClient, Environment environment) {
  ...
  protected FundingSources processInput(Void input, RequestInfo requestInfo, Context context)
    ...
    var sources =
        cristinClient.queryFundingSources().stream()
