...
public class FetchFundingSourcesHandlerTest {
  ...
  public void beforeEach(WireMockRuntimeInfo runtimeInfo) {
    ...
    when(environment.readEnv(ENV_KEY_DOMAIN_NAME))
        .thenReturn("https://api.sandbox.nva.aws.unit.no");
    ...
    var cristinClient =
        new CristinFundingSourcesApiClient(
            WiremockHttpClient.create(), URI.create(runtimeInfo.getHttpBaseUrl()));
  ...
  public void shouldReturnFundingSourceWhenExists(String acceptHeaderValue) throws IOException {
    try (var input =
        new HandlerRequestBuilder<Void>(dtoObjectMapper)
            .withPathParameters(Map.of(IDENTIFIER, EXISTING_FUNDING_SOURCE_IDENTIFIER_URL_ENCODED))
    ...
    var expectedId =
        URI.create(
            "https://api.sandbox.nva.aws.unit.no/cristin/funding-sources/"
  ...
  @Test
  public void shouldReturnStatusCodeNotFoundWhenFundingSourceDoesNotExistInCristin()
      throws IOException {
  ...
  public void shouldReturnBadGatewayWhenCristinIsUnavailable() throws IOException {
    ...
    var cristinClient =
        new CristinFundingSourcesApiClient(
            WiremockHttpClient.create(), URI.create("https://localhost:9999"));
    this.handlerUnderTest = new FetchFundingSourceHandler(cristinClient, environment);
  ...
  public void shouldUseProvidedBasePathForResourceIds() throws IOException {
    ...
    var expectedId =
        URI.create(
            "https://api.sandbox.nva.aws.unit.no/alternate-base-path/funding-sources/"
                + EXISTING_FUNDING_SOURCE_IDENTIFIER_URL_ENCODED);
    assertThat(fundingSource.id(), is(equalTo(expectedId)));
  }
  ...
  public void shouldSupportEmptyBasePathInResourceIds() throws IOException {
    ...
    var expectedId =
        URI.create(
            "https://api.sandbox.nva.aws.unit.no/funding-sources/"
                + EXISTING_FUNDING_SOURCE_IDENTIFIER_URL_ENCODED);
    assertThat(fundingSource.id(), is(equalTo(expectedId)));
  }
  ...
  private void assertNamesArePresentForFundingSource(FundingSource fundingSource) {
    ...
    assertThat(
        fundingSource.labels(),
        hasEntry(equalTo("en"), equalTo(EXISTING_FUNDING_SOURCE_IDENTIFIER)));
    assertThat(
        fundingSource.labels(),
        hasEntry(equalTo("nn"), equalTo(EXISTING_FUNDING_SOURCE_IDENTIFIER)));
    assertThat(
        fundingSource.labels(),
        hasEntry(equalTo("nb"), equalTo(EXISTING_FUNDING_SOURCE_IDENTIFIER)));
  }
}
