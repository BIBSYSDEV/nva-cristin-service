...
class QueryCristinOrganizationHandlerTest {
  ...
  public static final String CRISTIN_QUERY_RESPONSE_SORTED_ORDER =
      "cristinQueryResponseSortedOrder.json";
  ...
  public static final String NOT_FOUND_LOG_MESSAGE =
  ...
  void setUp() throws ApiGatewayException {
    ...
    queryCristinOrganizationHandler =
        new QueryCristinOrganizationHandler(clientProvider, new Environment());
  }

  @Test
  void shouldReturnAllResultsWhenNoParamsAreSpecified() throws Exception {
    var fakeQueryResponseResource =
        IoUtils.stringFromResources(Path.of(CRISTIN_QUERY_RESPONSE_V_2_JSON));
  ...
  void shouldReturnResponseOnQuery() throws Exception {
    ...
    doReturn(getOrganization("org_18_53_18_14.json"))
        .when(cristinApiClientVersionOne)
        .getOrganization(first);
    final var second = getCristinUri("2012.9.20.0", UNITS_PATH);
    doReturn(getOrganization("org_2012_9_20_0.json"))
        .when(cristinApiClientVersionOne)
        .getOrganization(second);
    ...
    doReturn(getTry(fakeHttpResponse))
        .when(cristinApiClientVersionOne)
        .sendRequestMultipleTimes(any());
  ...
  @ParameterizedTest(
      name = "Requesting version {0} should call version one {1} times and version two {2} times")
  @MethodSource("versionArgumentProvider")
  void shouldReturnCorrectVersionWhenRequestingItUsingQueryParam(
      String requestedVersion, int callsVersionOne, int callsVersionTwo) throws Exception {
    queryCristinOrganizationHandler =
        new QueryCristinOrganizationHandler(clientProvider, new Environment());
  ...
  void shouldReturnCorrectPayloadWhenRequestingVersion20230526() throws Exception {
    ...
    queryCristinOrganizationHandler =
        new QueryCristinOrganizationHandler(clientProvider, new Environment());
    var input =
        generateValidHandlerRequestWithVersionParam(
  ...
  void shouldHaveCorrectCristinUriWithParamsDifferentVersions(String apiVersion) throws Exception {
    queryCristinOrganizationHandler =
        new QueryCristinOrganizationHandler(clientProvider, new Environment());
    var input =
        generateHandlerRequestWithAdditionalQueryParameters(
    ...
    assertThat(
        captor.getValue().toString(), containsString("https://api.cristin-test.uio.no/v2/units?"));
  ...
  void shouldThrowExceptionWhenEnrichingFailWith5xxWhenWantingFullTree() throws Exception {
    ...
    doReturn(new HttpResponseFaker(EMPTY_OBJECT, HTTP_BAD_GATEWAY))
        .when(fetchClient)
        .fetchGetResult(any());
  ...
  void shouldReturnHitsInSortedOrderForVersion20230526() throws Exception {
    var fakeQueryResponseResource =
        IoUtils.stringFromResources(Path.of(CRISTIN_QUERY_RESPONSE_SORTED_ORDER));
    doReturn(new HttpResponseFaker(fakeQueryResponseResource))
        .when(queryCristinOrgClient20230526)
        .fetchQueryResults(any());
  ...
  void shouldConvertSortParamBetweenNvaFormatAndCristinFormat(
      String apiVersion, String nvaSort, String cristinSort) throws Exception {

    queryCristinOrganizationHandler =
        new QueryCristinOrganizationHandler(clientProvider, new Environment());
    var input =
        generateHandlerRequestWithSortParameters(
            format(ACCEPT_HEADER_EXAMPLE, apiVersion), nvaSort);
    queryCristinOrganizationHandler.handleRequest(input, output, context);
  ...
  private InputStream generateValidHandlerRequestWithVersionParam(String versionParam)
    ...
    return new HandlerRequestBuilder<InputStream>(restApiMapper)
        .withHeaders(
            Map.of(
                CONTENT_TYPE, APPLICATION_JSON_LD.toString(), ACCEPT_HEADER_KEY_NAME, versionParam))
  ...
  private Organization getOrganization(String subUnitFile) throws JsonProcessingException {
    return OBJECT_MAPPER.readValue(
        IoUtils.stringFromResources(Path.of(subUnitFile)), Organization.class);
  }
  ...
  private InputStream generateHandlerRequestWithRandomQueryParameter()
      throws JsonProcessingException {
    return new HandlerRequestBuilder<InputStream>(restApiMapper)
        .withHeaders(Map.of(CONTENT_TYPE, APPLICATION_JSON_LD.toString()))
  ...
  private InputStream generateHandlerRequestWithIllegalDepthParameter()
      throws JsonProcessingException {
    return new HandlerRequestBuilder<InputStream>(restApiMapper)
        .withHeaders(Map.of(CONTENT_TYPE, APPLICATION_JSON_LD.toString()))
  ...
  private FetchCristinOrgClient20230526 mockFetchClient() throws ApiGatewayException {
    ...
    var fakeGetSubsResponseResource =
        IoUtils.stringFromResources(Path.of(CRISTIN_GET_RESPONSE_SUB_UNITS_JSON));
  ...
  private InputStream handlerRequestWantingFullTree() throws JsonProcessingException {
    return new HandlerRequestBuilder<InputStream>(restApiMapper)
        .withHeaders(
            Map.of(
                CONTENT_TYPE,
                APPLICATION_JSON_LD.toString(),
                ACCEPT_HEADER_KEY_NAME,
                format(ACCEPT_HEADER_EXAMPLE, VERSION_2023_05_26)))
        .withQueryParameters(
            Map.of(
                QUERY, MEDICAL_BIOCHEMISTRY,
                DEPTH, FULL,
                FULL_TREE, Boolean.TRUE.toString()))
        .build();
  }

  private FetchCristinOrgClient20230526 mockFetchClientWithOneHitMissingInUpstream()
  ...
  private static Stream<Arguments> sortAndVersionArgumentProvider() {
    return Stream.of(
        Arguments.of(VERSION_ONE, "nameEn", "name_en"),
        Arguments.of(VERSION_ONE, "nameEn asc", "name_en asc"),
        Arguments.of(VERSION_ONE, "nameEn desc", "name_en desc"),
        Arguments.of(VERSION_2023_05_26, "nameEn", "name_en"),
        Arguments.of(VERSION_2023_05_26, "nameEn asc", "name_en asc"),
        Arguments.of(VERSION_2023_05_26, "nameEn desc", "name_en desc"),
        Arguments.of(VERSION_ONE, "nameNb", "name_nb"),
        Arguments.of(VERSION_ONE, "nameNb asc", "name_nb asc"),
        Arguments.of(VERSION_ONE, "nameNb desc", "name_nb desc"),
        Arguments.of(VERSION_2023_05_26, "nameNb", "name_nb"),
        Arguments.of(VERSION_2023_05_26, "nameNb asc", "name_nb asc"),
        Arguments.of(VERSION_2023_05_26, "nameNb desc", "name_nb desc"));
  }

  private InputStream generateHandlerRequestWithSortParameters(
      String versionParam, String nvaSortParam) throws JsonProcessingException {

    return new HandlerRequestBuilder<InputStream>(restApiMapper)
        .withHeaders(
            Map.of(
                CONTENT_TYPE, APPLICATION_JSON_LD.toString(), ACCEPT_HEADER_KEY_NAME, versionParam))
        .withQueryParameters(Map.of(QUERY, "strangeQueryWithoutHits", SORT, nvaSortParam))
        .build();
  }
}
