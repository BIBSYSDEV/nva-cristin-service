...
public class FetchCristinOrgClient20230526 extends ApiClient
  ...
  /**
   * Fetch one organization matching given query criteria. By specifying query param depth one can
   * choose if the response should include sub organization tree downwards or just have the upwards
   * parent trail.
   *
  ...
  public Organization executeFetch(Map<String, String> params) throws ApiGatewayException {
    ...
    checkHttpStatusCode(
        getNvaApiId(identifier, ORGANIZATION_PATH), response.statusCode(), response.body());

    if (wantsDepth(params)) {
      var fetchSubsUri =
          createCristinQueryUri(translateParamsForSubUnits(identifier, FIRST_PAGE), UNITS_PATH);
  ...
  private URI getCristinUri(String identifier) {
    return UriWrapper.fromUri(CRISTIN_API_URL).addChild(UNITS_PATH).addChild(identifier).getUri();
  }
  ...
  private List<UnitDto> deserializeSubUnits(HttpResponse<String> responseWithSubs)
      throws BadGatewayException {
    return asList(getDeserializedResponse(responseWithSubs, UnitDto[].class));
  }

  private Organization getMultiLevelOrganization(
      HttpResponse<String> response, List<UnitDto> subUnitsDto) throws BadGatewayException {
  ...
  private Organization getSingleLevelOrganization(HttpResponse<String> response)
      throws BadGatewayException {
    var unit = getDeserializedResponse(response, UnitDto.class);
