...
public class UpdateCristinPersonHandlerTest {
  ...
  private static final String NVI_JSON_AT_185 =
      IoUtils.stringFromResources(Path.of("nvaApiPersonNviData.json"));
  private static final String PERSON_CRISTIN_ID = "123456";
  private static final URI PERSON_CRISTIN_ID_URI =
      UriWrapper.fromUri(randomUri()).addChild(PERSON_CRISTIN_ID).getUri();
  ...
  void shouldReturnBadRequestWhenPrimaryNameIsNull() throws IOException {
    ...
    assertThat(
        ...
        containsString(String.format(FIELD_CAN_NOT_BE_ERASED, FIRST_NAME)));
  }
  ...
  void shouldReturnBadRequestWhenInvalidIdentifier() throws IOException {
    ...
    var gatewayResponse = sendQuery(Map.of(PERSON_ID, INVALID_IDENTIFIER), jsonObject.toString());

    assertEquals(HTTP_BAD_REQUEST, gatewayResponse.getStatusCode());
    assertThat(
        gatewayResponse.getBody(),
        containsString(ErrorMessages.invalidPathParameterMessage(PERSON_ID)));
  }
  ...
  void shouldReturnNoContentResponseWhenUserIsUpdatingTheirOwnPersonDataThatTheyAreAllowedToUpdate()
    ...
    jsonObject.put(ORCID, VALID_ORCID);
    var gatewayResponse =
        queryWithPersonCristinIdButNoAccessRights(validPath, jsonObject.toString());
  ...
  @Test
  void
      shouldThrowBadRequestWhenUsersWantToUpdateTheirOwnPersonDataButSendsFieldsTheyAreNotAllowedToUpdate()
          throws IOException {
    var jsonObject = OBJECT_MAPPER.createObjectNode();
    jsonObject.put(FIRST_NAME, randomString());
  ...
  void shouldThrowBadRequestWhenPathPersonIdentifierNotMatchingCognitoPersonIdentifier()
    ...
    jsonObject.put(ORCID, VALID_ORCID);
    var gatewayResponse =
        queryWithPersonCristinIdButNoAccessRights(
            Map.of(PERSON_ID, IDENTIFIER_NOT_MATCHING_COGNITO), jsonObject.toString());
  ...
  @Test
  void shouldFilterOutNviWhenClientNotAllowedToUpdateAtThatOrganizationForNviVerification()
      throws Exception {
  ...
  @Test
  void shouldNotFilterOutNviWhenClientIsAllowedToUpdateAtThatOrganizationForNviVerification()
      throws Exception {
    apiClient = spy(apiClient);
    handler = new UpdateCristinPersonHandler(apiClient, environment);
  ...
  private GatewayResponse<Void> sendQueryAsInstAdminWithSomeOrgId(String body, URI orgId)
      throws IOException {
  ...
  private GatewayResponse<Void> sendQuery(Map<String, String> pathParam, String body)
      throws IOException {
  ...
  private InputStream createRequest(
      String body, String scope, URI orgId, AccessRight... accessRight)
  ...
  private InputStream createRequest(Map<String, String> pathParam, String body)
      throws JsonProcessingException {
    var customerId = randomUri();
    return new HandlerRequestBuilder<String>(OBJECT_MAPPER)
        .withBody(body)
  ...
  private GatewayResponse<Void> queryWithPersonCristinIdButNoAccessRights(
      Map<String, String> pathParameters, String body) throws IOException {
    try (var input =
        new HandlerRequestBuilder<String>(OBJECT_MAPPER)
            .withBody(body)
