...
public class CristinPersonPatchJsonCreatorTest {
  ...
  @Test
  void shouldParseInputJsonIntoCristinJsonWithCorrectMappingOfFieldsAsAuthorizedClient()
      throws Exception {
    ...
    var expectedString =
        IoUtils.stringFromResources(Path.of("cristinUpdatePersonAsAdminRequest.json"));
  ...
  void shouldAddAllSupportedFieldsForPersonModifyingThemselves() throws Exception {
    ...
    var input = (ObjectNode) OBJECT_MAPPER.readTree(inputString);
    var result =
        new CristinPersonPatchJsonCreator(input).createWithAllowedUserModifiableData().getOutput();
