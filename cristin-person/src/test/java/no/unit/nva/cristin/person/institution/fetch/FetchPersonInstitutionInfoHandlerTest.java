...
public class FetchPersonInstitutionInfoHandlerTest {
  ...
  @Test
  void
      shouldReturnPersonInstitutionInfoWhenCallingEndpointWithValidPersonAndInstitutionIdentifiers()
    ...
    when(clientMock.<String>send(any(), any()))
        .thenReturn(new HttpResponseFaker(responseBody, 200));
    apiClient = new FetchPersonInstitutionInfoClient(clientMock);
    handler = new FetchPersonInstitutionInfoHandler(apiClient, environment);
  ...
  void shouldThrowBadRequestOnInvalidOrganizationPathParam() throws IOException {
    ...
    assertEquals(
        APPLICATION_PROBLEM_JSON.toString(),
        gatewayResponse.getHeaders().get(HttpHeaders.CONTENT_TYPE));
    assertThat(gatewayResponse.getBody(), containsString(invalidPathParameterMessage(ORG_ID)));
  }
  ...
  private static URI getExpectedId() {
    return new UriWrapper(HTTPS, DOMAIN_NAME)
        ...
        .addChild(VALID_PERSON_ID)
        .addChild(ORGANIZATION_PATH)
        .addChild(VALID_INSTITUTION_ID)
        .getUri();
  }

  private GatewayResponse<PersonInstitutionInfo> queryWithoutRequiredAccessRights()
      throws IOException {
    try (var input =
  ...
  private GatewayResponse<PersonInstitutionInfo> queryWithUnsupportedQueryParams()
      throws IOException {
