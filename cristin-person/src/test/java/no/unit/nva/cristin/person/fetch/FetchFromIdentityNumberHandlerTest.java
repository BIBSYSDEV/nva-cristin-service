...
public class FetchFromIdentityNumberHandlerTest {
  ...
  @Test
  void shouldProduceCristinUriWithCristinIdFromNationalIdentifier()
  ...
  @Test
  void shouldReturnServerErrorWhenBackendAuthenticationNotSentToUpstreamOrNotValid()
      throws Exception {
    var clientMock = mock(HttpClient.class);
    when(clientMock.<String>send(any(), any()))
        .thenReturn(new HttpResponseFaker(EMPTY_STRING, 401));
  ...
  void shouldReturnNotFoundWhenNoMatch() throws ApiGatewayException, IOException {
    apiClient = spy(apiClient);
    doReturn(new HttpResponseFaker(EMPTY_ARRAY, 200))
        .when(apiClient)
  ...
  private InputStream requestWithParams(
      TypedValue body, Map<String, String> queryParams, AccessRight accessRight)
  ...
  private GatewayResponse<Person> sendQueryWithAccessRight(TypedValue body, AccessRight accessRight)
      throws IOException {
