...
public class UpdatePictureHandlerTest {
  ...
  private static final URI PERSON_IDENTIFIER_MATCHING_ANOTHER_URI =
      getNvaApiId(ANOTHER_PERSON_CRISTIN_ID, PERSON);
  ...
  void shouldThrowUnauthorizedExceptionWhenClientIsNotAuthenticated() throws IOException {
    ...
    assertEquals(
        APPLICATION_PROBLEM_JSON.toString(), gatewayResponse.getHeaders().get(CONTENT_TYPE));
  }
  ...
  private GatewayResponse<Void> queryWithoutRequiredAccessRights(Binary body) throws IOException {
    var input =
        new HandlerRequestBuilder<Binary>(OBJECT_MAPPER)
            .withBody(body)
  ...
  private GatewayResponse<Void> queryWithValidBodyAndMatchingIdentifier(Binary body)
      throws IOException {
    return sendQuery(body, PERSON_IDENTIFIER_URI);
  }

  private GatewayResponse<Void> queryWithValidBodyButNonMatchingIdentifier(Binary body)
      throws IOException {
