...
public class QueryCristinPersonHandler extends CristinQueryHandler<Void, SearchResponse<Person>> {
  ...
  private static final Set<String> VALID_QUERY_PARAMETERS =
      Set.of(
          NAME,
          ORGANIZATION,
          PAGE,
          ...
          SORT);
  private final transient ClientProvider<CristinAuthorizedQueryClient<Map<String, String>, Person>>
      clientProvider;
  ...
  protected String getValidName(RequestInfo requestInfo) {
    return requestInfo.getQueryParameterOpt(NAME).map(UriUtils::decodeUri).orElse(null);
  }

  private String extractOrganization(RequestInfo requestInfo) {
    return requestInfo.getQueryParameterOpt(ORGANIZATION).map(UriUtils::decodeUri).orElse(null);
  }
  ...
  private String getSort(RequestInfo requestInfo) {
    return requestInfo.getQueryParameterOpt(SORT).orElse(null);
  }
