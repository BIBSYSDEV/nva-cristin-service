...
public class UpdatePictureAccessCheck {
  ...
  public static final String COULD_NOT_RETRIEVE_CLIENT_PERSON_ID =
      "Could not retrieve client person id from request";
  ...
  private Optional<String> getValidPersonIdFromCognito(RequestInfo requestInfo)
  ...
  private void validateUserIsThemselvesOrElseDenyAccess(
      String personIdFromPath, String personIdFromCognito) throws ForbiddenException {

    if (!Objects.equals(personIdFromPath, personIdFromCognito)) {
      logger.info(
          USER_TRYING_TO_UPDATE_ANOTHER_PERSONS_PICTURE, personIdFromCognito, personIdFromPath);
