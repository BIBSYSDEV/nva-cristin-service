...
public class QueryPersonEmploymentClient extends ApiClient {
  ...
  public static final String EMPLOYMENT_QUERY_CONTEXT =
      "https://example.org/person-employment-search-context.json";
  public static final String BAD_REQUEST_FROM_UPSTREAM =
      "Upstream returned Bad Request. This might occur if "
  ...
  public SearchResponse<Employment> generateQueryResponse(String identifier)
  ...
  private static List<Employment> mapEmploymentsToNvaFormat(
      String identifier, List<CristinPersonEmployment> cristinEmployments) {
    return cristinEmployments.stream()
        .map(cristinEmployment -> cristinEmployment.toEmployment(identifier))
        .collect(Collectors.toList());
  }

  private void checkResponseForBadRequestIndicatingNotFoundIdentifier(int statusCode)
