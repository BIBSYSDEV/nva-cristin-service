...
public class CreatePersonEmploymentClient extends PostApiClient {
   ...
   */
  public Employment createEmploymentInCristin(
      String identifier, Employment employment, String cristinInstitutionNumber)
  ...
  private URI getCristinPostUri(String identifier) {
    return UriWrapper.fromUri(getCristinUri(identifier, PERSON_PATH))
        .addChild(EMPLOYMENT_PATH_CRISTIN)
        .getUri();
  }

  private URI getNvaPostUri(String identifier) {
    return UriWrapper.fromUri(getNvaApiId(identifier, PERSON_PATH_NVA))
        .addChild(EMPLOYMENT_PATH)
        .getUri();
  }

  private CristinPersonEmployment deserializeResponse(HttpResponse<String> response)
