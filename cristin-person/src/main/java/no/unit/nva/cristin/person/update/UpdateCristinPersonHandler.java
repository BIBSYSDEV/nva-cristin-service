...
public class UpdateCristinPersonHandler extends ApiGatewayHandler<String, Void> {
  ...
  public static final String ERROR_MESSAGE_IDENTIFIERS_DO_NOT_MATCH =
      "Identifier from path does not match " + "identifier from user info";
  ...
  public UpdateCristinPersonHandler(
      UpdateCristinPersonApiClient apiClient, Environment environment) {
    ...
    if (clientCanUpdateAllFields(requestInfo)) {
      ...
      if (cristinJson.has(CRISTIN_EMPLOYMENTS) || cristinJson.has(PERSON_NVI)) {
        return apiClient.updatePersonInCristin(
            personId, cristinJson, extractCristinInstitutionIdentifier(requestInfo));
      } else {
  ...
  private static void logFailedAuthorization(RequestInfo requestInfo, String subject)
      throws UnauthorizedException {
    logger.error(
        "Unauthorized attempt to update person with id: {} by logged in user id {}",
        subject,
  ...
  private ObjectNode filterInput(RequestInfo requestInfo, ObjectNode objectNode) {
    var clientInstNr =
        attempt(() -> extractCristinInstitutionIdentifier(requestInfo)).orElse(fail -> null);

    return new PersonPatchFieldFilter(objectNode).filterOnInstNr(clientInstNr).getFiltered();
  }
  ...
  private void checkIdentifiersMatch(String personId, String personIdFromCognito)
  ...
  private Optional<String> parseLastPartOfPersonCristinIdFromCognito(RequestInfo requestInfo)
      throws UnauthorizedException {
    return Optional.ofNullable(requestInfo.getPersonCristinId())
  ...
  private void validateHasAccessRights(RequestInfo requestInfo)
      throws ForbiddenException, UnauthorizedException {
    if (!AccessUtils.requesterIsUserAdministrator(requestInfo)
        && !clientCanUpdateOwnData(requestInfo)) {
      var personIdFromQuery =
          attempt(() -> getValidPersonId(requestInfo)).orElse(e -> PERSON_ID_NOT_FOUND);
