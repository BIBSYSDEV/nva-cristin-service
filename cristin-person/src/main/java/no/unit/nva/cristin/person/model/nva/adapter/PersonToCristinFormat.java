...
public class PersonToCristinFormat implements Function<Person, CristinPerson> {
  ...
  public CristinPerson apply(Person person) {
    ...
    if (nonNull(person.nvi())) {
      var cristinPersonNvi =
          new CristinPersonNvi(
              extractVerifiedById(person.nvi()), extractVerifiedAtId(person.nvi()), null);
  ...
  private CristinNviInstitutionUnit extractVerifiedAtId(PersonNvi nvi) {
    ...
    return new CristinNviInstitutionUnit(
        null, CristinUnit.fromCristinUnitIdentifier(organizationHavingVerifiedId));
  }
  ...
  private List<CristinPersonEmployment> mapEmploymentsToCristinEmployments(
      Set<Employment> employments) {
    if (isNull(employments)) {
      return null;
    }

    return employments.stream().map(Employment::toCristinEmployment).toList();
  }
}
