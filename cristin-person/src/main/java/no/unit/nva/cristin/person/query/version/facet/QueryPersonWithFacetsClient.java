...
public class QueryPersonWithFacetsClient extends CristinPersonApiClient
  ...
  public SearchResponse<Person> executeQuery(Map<String, String> requestQueryParams)
    ...
    var convertedFacets =
        new CristinFacetConverter(id).convert(mappedResponse.facets()).getConverted();
  ...
  /**
   * Returns the same data as the open data query. We do not support query with authorized data in
   * this version as facet search is meant only for open data for the time being
   */
  @Override
  ...
  /**
   * Perform a query for Persons matching criteria in parameters. Appends facet parameters to query
   * uri if present.
   *
  ...
  private CristinPersonSearchResponse deserializeResponse(HttpResponse<String> response)
  ...
  /**
   * Enrich list of CristinPerson from query with more data by doing one extra lookup request per
   * person.
   */
