...
class ApiClientTest {
  ...
  private void
    ...
    doReturn(badResponse, badResponse, okResponse).when(httpClient).send(any(), any());

    apiClient = new ApiClient(httpClient);
  }

  private void mockResponseWithOnlyErrorStatusCodeOnAllAttempts()
      throws IOException, InterruptedException {
    doReturn(badResponse, badResponse, badResponse).when(httpClient).send(any(), any());
  ...
  private void mockResponseThatThrowsExceptionOnAllRequests()
      throws IOException, InterruptedException {
    doThrow(ioException())
        .doThrow(timeoutException())
  ...
  private void mockResponseThatThrowsExceptionOnTheFirstAttemptsButThenSucceedsAsync() {
    ...
    doThrow(uncheckedException)
        .doThrow(uncheckedException)
        .doReturn(okResponseAsync)
        .when(httpClient)
        .sendAsync(any(), any());
