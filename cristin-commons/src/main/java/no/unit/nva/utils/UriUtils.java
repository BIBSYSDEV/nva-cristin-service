...
public final class UriUtils {
  ...
  public static URI getCristinUri(String identifier, String path) {
    return UriWrapper.fromUri(CRISTIN_API_URL).addChild(path).addChild(identifier).getUri();
  }
   ...
   */
  public static URI createCristinQueryUri(
      Map<String, String> cristinRequestQueryParams, String path) {
  ...
  /**
   * Create position code uri where the position code is the code for an employment title in
   * Cristin.
   */
  public static URI createNvaPositionId(String code) {
    URI positionBase =
        new UriWrapper(HTTPS, DOMAIN_NAME).addChild(BASE_PATH).addChild(POSITION).getUri();
    return attempt(
            () ->
                new URI(
                    positionBase.getScheme(), positionBase.getHost(), positionBase.getPath(), code))
        .orElseThrow();
  }
