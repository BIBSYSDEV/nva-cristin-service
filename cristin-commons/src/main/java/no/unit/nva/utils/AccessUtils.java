...
public class AccessUtils {
   ...
   * @throws ForbiddenException thrown when user is not authorized to access a user with
   *     IdentificationNumber
   */
  public static void validateIdentificationNumberAccess(RequestInfo requestInfo)
  ...
  /**
   * Checks if the requester is permitted to act as a user administrator, either by having that
   * specific role, or if the client is internal backend.
   *
  ...
  private static URI getCognitoTokenUrl() {
    ...
    return UriWrapper.fromHost(cognitoAuthenticationDomain)
        .addChild(COGNITO_TOKEN_ENDPOINT)
        .getUri();
  }
