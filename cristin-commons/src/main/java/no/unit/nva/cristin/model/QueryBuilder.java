...
public abstract class QueryBuilder<T extends Enum<T> & IParameterKey> {
   ...
   *
   * <p>Usage: <samp>new CristinQuery.Builder()<br>
   ...
   *
   * <p>Usage: <samp>var nonNullValue = nonNull(value) ? value : EMPTY_STRING;<br>
  ...
  protected Set<String> getMissingKeys() {
    return requiredMissing().stream().map(IParameterKey::getNvaKey).collect(Collectors.toSet());
  }

  protected Set<T> required() {
    return Stream.concat(query.otherRequiredKeys.stream(), query.pathParameters.keySet().stream())
