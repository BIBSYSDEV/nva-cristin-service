...
public final class Utils {
  ...
  public static final String USER_TOP_LEVEL_CRISTIN_ORGANIZATION =
      "User has top level cristin organization {}";
  ...
  public static <T> Predicate<T> distinctByKey(Function<? super T, ?> keyExtractor) {
    return t ->
        new ConcurrentHashMap<Object, Boolean>().putIfAbsent(keyExtractor.apply(t), Boolean.TRUE)
            == null;
  }
  ...
  public static String getValidPersonId(RequestInfo requestInfo) throws BadRequestException {
    String identifier =
        attempt(() -> requestInfo.getPathParameter(PERSON_ID)).orElse(fail -> EMPTY_STRING);
  ...
  public static String getValidEmploymentId(RequestInfo requestInfo) throws BadRequestException {
    String identifier =
        attempt(() -> requestInfo.getPathParameter(EMPLOYMENT_ID)).orElse(fail -> EMPTY_STRING);
    if (isValidIdentifier(identifier)) {
      return identifier;
    }
  ...
  /**
   * Validates if organization identifier path parameter exists and is valid format. If supplied
   * parameter is unit identifier, the method strips it down to top level institution identifier as
   * defined in Cristin format.
   *
  ...
  public static String getValidOrgId(RequestInfo requestInfo) throws BadRequestException {
    String identifier =
        attempt(() -> requestInfo.getPathParameter(ORG_ID)).orElse(fail -> EMPTY_STRING);
  ...
  /**
   * Extracts institution string composed of digits from data stored in RequestInfo to be used as
   * value in a header sent to upstream which describes which institution the user is allowed to
   * change certain data at.
   *
   * @param requestInfo information from request used to verify allowed permissions
   * @return String with allowed institution to change. Or constant 'any' if internal backend with
   *     full access
  ...
  public static String extractCristinInstitutionIdentifier(RequestInfo requestInfo)
    ...
    attempt(() -> Integer.valueOf(institution))
        .orElseThrow(fail -> failedToRetrieveTopLevelOrgCristinId());
  ...
  public static String getValidIdentifier(RequestInfo requestInfo) throws BadRequestException {
    attempt(() -> Integer.parseInt(requestInfo.getPathParameter(IDENTIFIER)))
        .orElseThrow(
            failure ->
                new BadRequestException(ERROR_MESSAGE_INVALID_PATH_PARAMETER_FOR_IDENTIFIER));
