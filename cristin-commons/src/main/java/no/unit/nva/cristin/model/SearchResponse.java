...
@JsonInclude(ALWAYS)
@JsonPropertyOrder({
  JsonPropertyNames.CONTEXT,
  JsonPropertyNames.ID,
  JsonPropertyNames.SIZE,
  JsonPropertyNames.SEARCH_STRING,
  JsonPropertyNames.PROCESSING_TIME,
  JsonPropertyNames.FIRST_RECORD,
  JsonPropertyNames.NEXT_RESULTS,
  JsonPropertyNames.PREVIOUS_RESULTS,
  JsonPropertyNames.HITS,
  JsonPropertyNames.AGGREGATIONS
})
public class SearchResponse<E> implements JsonSerializable {
  ...
  @JsonIgnore public static final int FIRST_RECORD_ZERO_WHEN_NO_HITS = 0;
  ...
  @JsonProperty private Long processingTime;
  @JsonProperty private Integer firstRecord;
  @JsonProperty private URI nextResults;
  @JsonProperty private URI previousResults;
  @JsonProperty private List<E> hits;
   ...
   */
  public SearchResponse<E> usingHeadersAndQueryParams(
      HttpHeaders headers, Map<String, String> queryParams) throws BadRequestException {

    this.size = getSizeHeader(headers);
    this.firstRecord =
        this.size > 0
            ? indexOfFirstEntryInPageCalculatedFromParams(queryParams)
