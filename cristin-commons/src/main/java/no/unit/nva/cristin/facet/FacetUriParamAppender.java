...
public class FacetUriParamAppender {
  ...
  public static final List<String> KEYS_THAT_SHOULD_NOT_BE_SPLIT =
      List.of("title", "query", "name");
  ...
  /**
   * Adds the facet name and facet key from input facet as a query param to the input nva uri. Facet
   * name is linked to the enum for Cristin facets. If the facet is already present, it adds the new
   * one to the existing query param as a comma separated list of facet keys. Returns a new uri with
   * the added facet data.
   */
  ...
  public FacetUriParamAppender create() {
    ...
    var queryParamMap =
        nonNull(queryParam) ? getQueryMap(queryParam) : new HashMap<String, List<String>>();
  ...
  private QueryParameter saveAndProcessNextFragment(
      String fragment, QueryParameter currentParam, Map<String, List<String>> parametersMap) {
