...
public class CristinOrganization {
  ...
  public Organization extractPreferredTypeOfOrganization() {
    var unit = Optional.ofNullable(getInstitutionUnit()).map(CristinUnit::toOrganization);
    var institution = Optional.ofNullable(getInstitution()).map(CristinInstitution::toOrganization);

    return unit.orElse(institution.orElse(null));
  }

  /**
   * Creates CristinOrganization from identifier. Either Unit which is preferred, or else falls back
   * to institution.
   */
