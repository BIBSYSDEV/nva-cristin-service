...
public class CristinFacetConverter {
  ...
  /**
   * Takes a facet map from Cristin and converts it to a facet map formatted for NVA. Unsupported
   * facets are silently ignored. Uses Nva id uri as input for calculating facet id uri.
   */
  ...
  private List<Facet> convertCristinFacetHavingKey(
      Map<String, CristinFacet[]> input, String cristinFacetKey) {
  ...
  private List<Facet> toFacets(String cristinFacetKey, CristinFacet... cristinFacets) {
    return Arrays.stream(cristinFacets)
        .map(cristinFacet -> toFacet(cristinFacetKey, cristinFacet))
        .toList();
  }
